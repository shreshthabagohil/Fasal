...
Judge call avg ~1,700 tokens => TPD = ~58 calls/day. Full dual-order eval on N=400 = 800 calls / 1.36M tokens => 13x over.

## Chosen lane: B (shrink held-out to N<=80 for iterative evals)

## Reason
Zero-budget rule (project hard constraint) rules out Lane C (paid Groq). Lane B is the smallest code change (subset the held-out at eval time, no new judge.py backend) and it protects the number that actually matters: the final M5 report still runs on the full N=400, so the headline significance claim stays fully powered. Only the M3/M4 iteration-to-iteration comparisons get wider CIs, which is acceptable since we are not claiming significance on those anyway.
